...
public class AtricaAdapter extends Adapter {
	...
	public IDeviceDetailsResponse fetch(String circuitId, String deviceIP, Integer snmpVersion, String community, String portName, String type, String serviceType, String ocn) throws Exception {
		...
		try {
			...
			IAspen aspen = (IAspen) Registry.bind(baseUrl + "/aptCache/services/Aspen.wsdl",IAspen.class);
					...
					if(deviceInfRes != null) {
						...
						if(port != null && port.length > 0 ){
							for(int i = 0; i < port.length ; i++) {
								...
								inf.setIpaddress(port[i].getDeviceId());
								inf.setStatus(port[i].getStatus());
			...
			if (deviceDetailsResponse.getErrorResponse() == null) {
				...
				try {
					errorResponse.setMessage(MessageFormat.format(MessagesErrors.getDefaultInstance().getProperty("error.snmp.atrica").trim(), e.toString()));
				} catch (Exception e1) {
					log.error(e1,e1);
				}
