	...
	private String[] emsapiFetch(Map<String,Object> input) {
		...
		if(input != null && deviceDetailsResponse != null && input.containsKey("deviceDetails")) {
			try {
				...
				IDeviceDetailsResponse ddr = adapter.fetch(deviceDetailsResponse.getCircuitID(), deviceDetailsResponse.getDeviceIP(), null, null, deviceDetails.getPortName(), deviceDetails.getType(), null, deviceDetails.getOcn());
